...
一款面向 macOS 的划词翻译、截图理解与个人词典应用。选中其他应用中的文本或框选屏幕区域，即可在翻译浮窗中获得 AI 翻译；有价值的表达和截图可以继续收藏、补充笔记，沉淀到应用首页的词汇列表中。
...
4. 点击「保存到词典」或使用已配置的保存快捷键，将结果加入应用首页的词汇列表；保存后可从提示中直接查看详情。
...
- 可另行配置仅在翻译浮窗内生效的「保存词汇到词典」快捷键，默认不设置。
- 自动读取当前应用中选中的文本并立即翻译。
- 支持手动输入、源语言与目标语言选择，以及一键交换翻译方向。
- 翻译结果包含自然译文，并按内容提供 1–2 条双语例句。
- 支持英语、简体中文、繁体中文、日语、韩语、法语、德语和西班牙语互译。
- 按 `Esc` 或再次使用快捷键即可关闭翻译浮窗。
- 菜单栏图标可快速打开翻译浮窗、恢复应用首页或退出程序。

### 截图理解与翻译

- 默认使用 `⇧⌘2` 唤起，也可以从菜单栏的「截图翻译」进入或在设置中重新录制快捷键。
- 唤起后会在所有屏幕显示蒙层；拖动框选区域，再点击「翻译」或按 Return 确认，Esc 可取消。
- 翻译浮窗会展示截图预览、整体 summary，并按画面阅读顺序逐项列出原文、译文和该文字元素的位置与作用。
- 截图前会检查当前模型的图片输入能力；不支持时会在当前屏幕中央提示，不会打开截图蒙层。
- 截图翻译可以保存到词汇列表；词汇卡片以缩略图代替原词，并用简短 summary 作为目标含义。
...
- 可从应用首页右上角的词汇表菜单导出 JSON 备份，或将备份安全合并到现有词汇列表。
- 导入时同一条词汇按更新时间保留较新版本，并兼容旧版裸数组文件及包含未知新字段的备份。
...
- 截图翻译需要支持图片输入的模型；当前 Qwen 配置可直接使用，纯文本模型会在唤起时提示不支持。

关闭应用首页只会隐藏窗口，不会退出 Translatora。再次点击 Dock 图标、菜单栏中的「打开应用首页」，或从保存成功提示进入词汇详情时，会自动恢复被隐藏或最小化的应用首页。需要完全退出时，请使用菜单栏的「退出 Translatora」、Dock 菜单或系统退出命令。
...
如果快捷键与其他应用冲突，可在设置中点击当前快捷键并直接录制新的组合，也可使用每行右侧的删除按钮清除配置。组合中至少需要一个修饰键。
...
- 发起截图翻译时，所选区域的图片会发送到当前选中的翻译服务；保存后图片会以 Base64 数据随词汇记录写入本地词典和导出的 JSON 备份。
- 辅助功能权限用于读取其他应用的选中文本。读取过程会触发复制操作，并在完成后恢复原有剪贴板内容。
- 屏幕录制权限仅用于截取用户主动框选的区域；确认框选前不会向模型发送图片。
...
截图选区 ── ScreenshotSelectionController ───────┘
                                      │
                                      ▼
